...
for ds in glove-100-angular glove-2m-300-angular; do
    ...
    for k in 5 10 50; do
        for kk in $(( $k * 2)) 100; do
            ...
            python3 additional-scripts/compute-expansion.py data/${ds}.hdf5 $k $kk > workloads/${ds}-${t}-${k}_${kk}.txt
            echo Picking query sets for $t on $ds
            python3 additional-scripts/choose-queryset.py --${t} workloads/${ds}-${t}-${k}_${kk}.txt > workloads/${ds}-${t}-${k}_${kk}-queries.txt
            python3 additional-scripts/pick-queries.py --${t} data/${ds}.hdf5 workloads/${ds}-${t}-${k}_${kk}-queries.txt
        done
    done
done
